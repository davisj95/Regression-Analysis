Jacob Davis
Feb 6, 2018
Simple Linear Regression - Diamonds


EDA - Class Preparation
```{r}
diamonds <- read.table(text="0.3  D VS2  GIA  1302
0.3  E VS1  GIA  1510
0.3  G VVS1 GIA  1510
0.3  G VS1  GIA  1260
0.31 D VS1  GIA  1641
...
1.02 H VS2  HRD  8959
1.09 I VVS2 HRD  9107", header=FALSE)
...
#For our purposes, using the ln of the data is more useful because it provides for us a model that we can calculate a simple linear model for. This would also mean that our variance would diminish and it takes points that seem to be outliers and bring them in so that we can use all of the data

```


model: ln(Price) = beta0 + beta1 log(Carat) + epsilon
```{r}
...
#predict for a 1 carat diamond (in ln price)
...
#Pretty Chart showing points, predicted price with prediction error
plot.df <- cbind(diamonds, predict(out.diamonds, interval="prediction"))

ggplot(plot.df, aes(log(Carat),log(Price))) + 
  xlab("Carat") + 
  geom_point() + 
